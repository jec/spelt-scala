...
package object requests:
  implicit val createRoomRequest: Reads[CreateRoomRequest] = (
    (JsPath \ "creation_content").readNullable[CreateRoomEvent] and
      (JsPath \ "initial_state").read[Seq[StateEvent]] and
      (JsPath \ "invite").read[Seq[String]] and
      (JsPath \ "invite_3pid").read[Seq[Invite3Pid]] and
      (JsPath \ "is_direct").readNullable[Boolean] and
      (JsPath \ "name").readNullable[String] and
      (JsPath \ "power_lever_content_override").read[Seq[PowerLevelEvent]] and
